...
def registrar_cliente_db(telefono, nombre, precio):
    """Upsert de cliente. Retorna el telefono como identificador."""
    ...
    try:
        ...
        return telefono   # ← retorna el telefono como FK
    except Exception as e:
...
def registrar():
    ...
    sb_insert('registros', {
        ...
        'cliente_telefono': telefono or None,   # ← FK a clientes
        'costo_estimado':   0,                  # ← actualizable después
    })
...
def crear_evento():
    ...
    sb_insert('eventos', {
        'tipo':             d.get('tipo', 'evento'),
        'cliente':          d.get('cliente'),
        'telefono':         d.get('telefono'),
        'festejado':        d.get('festejado'),
        'anos':             d.get('anos'),
        'ninos_aprox':      d.get('ninos_aprox'),
        'fecha_evento':     d.get('fecha_evento'),
        'lugar':            d.get('lugar'),
        'servicios':        d.get('servicios'),
        'observaciones':    d.get('observaciones'),
        'precio_total':     int(d.get('precio_total', 0)),
        'anticipo':         int(d.get('anticipo', 0)),
        'estado':           d.get('estado', 'cotizado'),   # ← default seguro
        'pago':             d.get('pago', ''),
        'cliente_telefono': d.get('telefono') or None,    # ← FK a clientes
        'costo_total':      int(d.get('costo_total', 0)), # ← nuevo campo
    })
    return jsonify({'ok': True})
    ...
    if supabase:
        try:
            sb_insert('ventas_jugueteria', {
                ...
                'cliente_telefono': data.get('telefono') or None,  # ← FK a clientes
            })
        except Exception as e:
